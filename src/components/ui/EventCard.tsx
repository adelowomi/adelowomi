...
const EventCard: React.FC<EventCardProps> = ({ event, onEdit, onView }) => {
  const [showPopup, setShowPopup] = useState(false);
  const [loading, setLoading] = useState(false);
  const { updateEvent, deleteEvent } = useAdminEvents();
  ...
  // Default values for when no event is provided (backward compatibility)
  ...
  const ticketsLeft =
    eventData.availableSpots || eventData.capacity - registrationCount;
  const progressPercentage =
    eventData.capacity > 0 ? (registrationCount / eventData.capacity) * 100 : 0;

  // Calculate days left (assuming registration closes on event date)
          ...
          {showPopup && (
            <div className="absolute top-8 right-0 bg-[#0D0900] border-none rounded-lg p-2.5 z-50 w-[116px]">
              <ul className="text-white font-archivo text-lg font-normal space-y-2">
                <li
                  className="cursor-pointer flex flex-row items-center gap-2.5 text-secondary"
                  onClick={handleView}
                >
                  <svg
                    width="16"
                ...
                <li
                  className="cursor-pointer flex flex-row items-center gap-2.5 text-[#0D07CB]"
                  onClick={handleEdit}
                >
                  <svg
                    width="16"
                ...
                {eventData.status === EventStatus.ACTIVE ? (
                  <li
                    className="cursor-pointer flex flex-row items-center gap-2.5 text-[#FFB616]"
                    onClick={() => handleStatusChange(EventStatus.INACTIVE)}
                    disabled={loading}
                  >
                    <svg
                      width="16"
                  ...
                  <li
                    className="cursor-pointer flex flex-row items-center gap-2.5 text-[#028F1C]"
                    onClick={() => handleStatusChange(EventStatus.ACTIVE)}
                    disabled={loading}
                  >
                    <svg
                      width="16"
                    ...
                    >
                      <path
                        d="M7.33366 13.3334V7.90002L5.60033 9.63335L4.66699 8.66669L8.00033 5.33335L11.3337 8.66669L10.4003 9.63335L8.66699 7.90002V13.3334H7.33366ZM2.66699 6.00002V2.66669H13.3337V6.00002H12.0003V4.00002H4.00033V6.00002H2.66699Z"
                        fill="#028F1C"
                      />
                    </svg>
                    Publish
                ...
                <li
                  className="cursor-pointer flex flex-row items-center gap-2.5 text-[#B40606]"
                ...
                >
                  ...
                  >
                    <path
                      d="M4.66699 14C4.30033 14 3.98655 13.8696 3.72566 13.6087C3.46477 13.3478 3.3341 13.0338 3.33366 12.6667V4H2.66699V2.66667H6.00033V2H10.0003V2.66667H13.3337V4H12.667V12.6667C12.667 13.0333 12.5365 13.3473 12.2757 13.6087C12.0148 13.87 11.7008 14.0004 11.3337 14H4.66699ZM11.3337 4H4.66699V12.6667H11.3337V4ZM6.00033 11.3333H7.33366V5.33333H6.00033V11.3333ZM8.66699 11.3333H10.0003V5.33333H8.66699V11.3333Z"
                      fill="#B40606"
                    />
                  </svg>
                  Delete
